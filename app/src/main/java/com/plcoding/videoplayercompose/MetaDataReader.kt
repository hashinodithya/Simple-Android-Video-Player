...
interface MetaDataReader {
    fun getContent(contentUri: Uri): MetaData?
}

class MetaDataReaderImp(
    private val app: Application
): MetaDataReader{
    override fun getContent(contentUri: Uri): MetaData? {
        if (contentUri.scheme!= "content"){
            return null
        }
        val fileName = app.contentResolver
            .query(contentUri,arrayOf(MediaStore.Video.VideoColumns.DISPLAY_NAME),
            null,
                null,
                null,
                )
